...
MyMesh::VertexHandle vhandle[6075]; // NOLINT
...
bool Visualize (MyMesh& mesh, const Mat &image, const String &name){
    ...
    // std::cout<<c;
    // std::cin>>c;
    cv::imwrite(  name + "_with_count.png" , image_pruned );
...
bool Visualize (MyMesh& mesh, const Mat &image, const String &name, std::list<std::pair<Point, Point>> deleted){
    int c = 0;
    cv::Mat_<Vec3b> image_pruned;
    image.copyTo(image_pruned);
    Point p, first, last, avg;
